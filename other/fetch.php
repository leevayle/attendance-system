...
// Obtain id_no from $_GET array
$id_no = isset($_GET['idNo']) ? $_GET['idNo'] : '';

// Query to check if the record exists in the clock table for the given id_no and today's date
$checkRecordQuery = "SELECT COUNT(*) AS recordCount FROM clock WHERE id_no = '$id_no' AND date = '$today'";
$recordResult = $conn->query($checkRecordQuery);

if ($recordResult->num_rows > 0) {
    $recordRow = $recordResult->fetch_assoc();
    $recordCount = $recordRow['recordCount'];
    
    // Write the status to localStorage based on the record existence
    if ($recordCount > 0) {
        // Record exists
        echo '<script>
            localStorage.setItem("thisstatus", "yes");
        </script>';
    } else {
        // Record doesn't exist
        echo '<script>
            localStorage.setItem("thisstatus", "no");
        </script>';
    }
}
